...
include 'ai_categorizer.php'; // <-- put aiCategorize() function in a separate file
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $category = aiCategorize($title . " " . $description);
    ...
    if (empty($upload_error)) {
        $stmt = $conn->prepare("INSERT INTO packages 
            (title, location, description, price, status, category, image1, image2, image3, image4, start_date, end_date, itinerary) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        
        $stmt->bind_param(
            "sss d s s s s s s s s s",
            $title,
            $location,
            $description,
            $price,
    ...
    <label>Status:
     ...
     <label>Category:</label>
        <select name="category">
            <option value="auto">Auto (AI Suggestion)</option>
            <option value="Beach">Beach</option>
            <option value="Mountain">Mountain</option>
            <option value="City Vibe">City Vibe</option>
